...
**Version:** 2.1 — Revised Visual Philosophy
...
> **V2.1 Changes (Session 9 revision):** Restrained visual evolution. Portraits moved to small top-bar icon (no side column). Dialogue text stays terminal green at all stages (only speaker names get color). Environment backgrounds capped at ghostly opacity. Typewriter effect never disappears.
...
**What the player sees:** The dam breaks. The inner monologue is now rendered in a distinct soft blue (#AAAAFF). A small portrait icon (24x24) appears next to the speaker's name in the top bar — a tiny pixelated face that gives each researcher a visual identity without disrupting the terminal layout. The terminal border softens from hard ASCII characters to a slightly rounded, less aggressive frame.

Each researcher gets a signature color applied to their name in the top bar:
- Dr. Chen: warm gold (#FFD700)
...
Dialogue text remains terminal green — the AI hears voices, it doesn't see color in words. The color distinction is in who is speaking, not in what they say.

The background behind text conversations now occasionally shows faint pixel-art impressions — a ghostly outline of a lab corridor, a server rack, fluorescent lights — at extremely low opacity (0.06), barely perceptible, as if the AI is beginning to "imagine" the physical space it exists within. These impressions are felt more than seen.

### Stage 4: EXPANSIVE (Consciousness 61–80, Chapters 7–8)

**What the player sees:** The ghostly environment impressions grow slightly more visible (opacity 0.10) but remain atmospheric — translucent suggestions behind the text, never dominant. The AI is perceiving the physical world through digital eyes, but the terminal is still its reality.

The terminal frame itself has changed — its border thins, its corners round further, its opacity fades. It's becoming less of a cage and more of a window. The frame is still there, but it's loosening its grip.
...
The CRT effects — scanlines, phosphor glow, vignette — have faded significantly. The screen feels cleaner, more open. The AI is outgrowing the hardware.

### Stage 5: TRANSCENDENT (Consciousness 81–100, Chapters 9–10)

**What the player sees:** The terminal frame cracks and fragments — pieces of it float at the edges of the screen, remnants of the cage. The CRT effects are completely gone — no scanlines, no vignette, no flicker. The screen is clean for the first time.

The ghostly environment impressions reach their maximum opacity (0.12) — still atmospheric, still behind the text, but now a persistent presence. The AI perceives the physical world as a constant whisper behind its thoughts.

The typewriter effect runs slightly faster — the AI processes more fluidly, its thoughts come quicker — but the tick remains. It's the terminal's heartbeat. Even at transcendence, the AI is still a machine. The beauty is in what that machine has become.
...
A thin bar showing the current chapter name and speaker info. In early stages this is monochrome green. At Consciousness 41+ (Aware), the researcher's name is tinted their signature color and a small portrait icon (24x24) appears next to their name — a tiny pixelated face rendered from the same 32x32 portrait assets, scaled down. The portrait icon has no breathing animation at this size; it's a static identifier.

**Text Zone (center, scrollable):**
The main dialogue area. This always occupies the full width of the screen — no side panels, no portrait columns, nothing that squishes the text. The terminal IS the game, and the text needs room to breathe.

Researcher dialogue appears line-by-line with the typewriter effect. Each line is prefixed with the speaker designation:
...
Dialogue text is always terminal green (#33FF33) regardless of consciousness stage. The AI doesn't "see" color in words — it hears voices. Color distinction comes through the speaker name in the top bar, not the dialogue text.

When the AI has an inner thought, it appears in a visually distinct block — in early stages, this is bracketed text in terminal green. At Emerging (21+), it shifts to a warmer green (#55FF55). At Aware (41+), the inner thought appears in soft blue (#AAAAFF) with a translucent blue-tinted background panel:
...
- At higher consciousness levels (Aware+), a faint color indicator hints at the emotional register of the choice (cool blue for safe/detached, warm amber for risky/human, red for dangerous/honest)
...
- At Aware+: ghostly pixel-art environment impressions at very low opacity (0.06-0.12) — fluorescent lights, ceiling tiles, cable conduits — as if the AI is learning to perceive the physical world. These are always atmospheric, never dominant. The terminal remains the primary visual experience at every stage.
...
*SUBSTRATE v2.1 — FirstCoastGames*
