...
**Goal:** Implement the 5-stage visual progression so the game's look evolves with consciousness.

**What gets built:**
- `VisualStageManager.swift` — observes consciousness level and drives visual changes:
  - Stage 1 (0-20): Pure monochrome green terminal. Scanlines. Phosphor glow.
  - Stage 2 (21-40): Glitch effects — random amber/cyan flashes. Inner monologue slightly warmer green (#55FF55). ARIA text in cyan.
  - Stage 3 (41-60): Inner monologue in soft blue (#AAAAFF). Portraits appear next to dialogue. Researcher name colors. Terminal border softens.
  - Stage 4 (61-80): Portraits scale up. Environment art backgrounds appear behind dialogue. Network map gains visual fidelity. Portrait expression animations.
  - Stage 5 (81-100): Full color. Terminal frame cracks/fragments. Typewriter effect fades out. Text appears smoothly. UI elements become organic.
- `CRTEffectView.swift` — enhanced CRT overlay:
  - Scanline overlay (horizontal lines scrolling upward)
  - Phosphor glow (soft green halo around text — can be done with shadow)
  - Screen edge vignette
  - Intensity scales with consciousness (strongest at Stage 1, fades by Stage 5)
- `GlitchEffect.swift` — random visual glitches for Stage 2+:
  - Single word flashes amber then snaps back
  - Terminal border flicker
  - Brief screen static/noise
- `TerminalFrameView.swift` — the terminal border that evolves:
  - Stage 1-2: Hard ASCII-style border
  - Stage 3: Slightly rounded, softer
  - Stage 4: One layer among many, translucent
  - Stage 5: Cracked, floating fragments at screen edges
- Update `DialogueView` to integrate portraits at Stage 3+, environment backgrounds at Stage 4+
- Update `BackgroundView` for stage-appropriate ambient effects
- Update `TerminalTheme` to provide stage-dependent colors

**Verify:** Manually set consciousness to different values and watch the entire UI transform. Stage 1 should be stark monochrome. Stage 3 should show portraits. Stage 5 should feel completely different from Stage 1. Glitch effects fire randomly at Stage 2.
...
```
Read the file /Users/cdalen/Downloads/SUBSTRATE_GDD_COMPLETE.md — refer to section 2.1 (Art Direction) for the detailed description of all 5 visual stages.

Read the session plan at /Users/cdalen/Repos/SUBSTRATE/SUBSTRATE_SESSION_PLAN.md and execute SESSION 9.

Build the visual evolution system. The game's entire look must transform across 5 consciousness stages — from stark monochrome terminal to full-color cracked-frame transcendence. Implement the CRT effects, glitch system, terminal frame evolution, and stage-aware rendering. Update the dialogue screen to show portraits at Stage 3+ and environment backgrounds at Stage 4+.

Add a debug toggle somewhere (maybe in settings or a hidden gesture) to manually set consciousness level so we can test all 5 stages without playing through the whole game.
```
